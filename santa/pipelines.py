...
class SantaPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `santa`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM santa WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO santa (date, title, link, content)"
